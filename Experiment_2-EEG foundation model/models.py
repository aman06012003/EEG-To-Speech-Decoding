...
class SpeechDecoder(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.use_phoneme_predictor = kwargs.get('use_phoneme_predictor', False)
    if self.use_phoneme_predictor:
        self.phoneme_predictor = PhonemePredictor(
            num_layers=kwargs.get('num_conformer_layers', 1),
            num_class=kwargs.get('vocab_size', 51),
            encoder_dim=inter_channels,
            activate_decoder=True
        )

  def forward(self, x, x_lengths, y, y_lengths, phoneme=None, alpha=1.0):
    ...
    # Phoneme prediction from speech latent
    phoneme_preds_speech = None
    if self.use_phoneme_predictor and phoneme is not None:
        phoneme_preds_speech = self.phoneme_predictor(z, y_lengths, phoneme)
    ...
    # return o, None, None, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q), sid_logits_speech
    return o, None, None, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q), phoneme_preds_speech
  ...
  def infer(self, x, x_lengths, noise_scale=0.667, max_len=None, alpha=1.0):
    ...
    #     z_for_subj = z_for_subj.mean(dim=2)
    # sid_logits_speech = self.speech_subject_discriminator(z_for_subj, alpha)
    # return o, None, y_mask, (z, z_p, m_p, logs_p), sid_logits_speech
    return o, None, y_mask, (z, z_p, m_p, logs_p), None
