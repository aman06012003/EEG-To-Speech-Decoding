...
class SpeechDecoder(nn.Module):
  ...
  def __init__(self, 
    ...
    use_speech_phoneme_predictor=False,
    vocab_size=51,
    encoder_dim=192,
    ...
    **kwargs):
    ...
    self.use_speech_phoneme_predictor = use_speech_phoneme_predictor
    if self.use_speech_phoneme_predictor:
  ...
  def forward(self, x, x_lengths, y, y_lengths, phoneme=None, phoneme_lengths=None):
    ...
    phoneme_preds = None
    if self.use_speech_phoneme_predictor and phoneme is not None:
        # z is [b, c, t], same as mid_output in EEGModule
        # However, z has same length as y (spec), which is different from x (eeg)
        # We need to use z (posterior) for phoneme prediction
        # z shape: [B, hidden_channels, T_spec]
        phoneme_preds = self.phoneme_predictor(z, y_lengths, phoneme)

    return o, None, None, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q), phoneme_preds
