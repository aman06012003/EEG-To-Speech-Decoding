...
def run(rank, n_gpus, hps):
  ...
  eeg_module = EEGModule(
    ...
    use_dora=hps.model.eeg_module.use_dora if hasattr(hps.model.eeg_module, 'use_dora') else False,
    lora_rank=hps.model.eeg_module.lora_rank if hasattr(hps.model.eeg_module, 'lora_rank') else 8,
    lora_alpha=hps.model.eeg_module.lora_alpha if hasattr(hps.model.eeg_module, 'lora_alpha') else 16
  ).cuda(rank)
  ...
  optim_eeg = torch.optim.AdamW(
      list(filter(lambda p: p.requires_grad, eeg_module.parameters())) + (list(net_p.parameters()) if net_p is not None else []), 
      hps.train.learning_rate, 
      betas=hps.train.betas, 
      eps=hps.train.eps)
  ...
  if hps.train.pretrained_audio:
    ...
    for k, v in net_g.module.state_dict().items():
      # Skip keys not present in pretrained checkpoint (like the new phoneme_predictor or proj layers)
      if k.split('.')[0] in ['enc_proj', 'subject_discriminator', 'speech_subject_discriminator', 'phoneme_predictor'] or k not in pretrained_stated_dict:
        new_state_dict[k] = v
      else:
  ...
  if True: # continue from past run
    try:
      ...
      pass
    for optim in [optim_g, optim_d, optim_eeg]:
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, batch in enumerate(train_loader):
    ...
    with autocast(enabled=False):
      # Calculate alpha for GRL
      alpha = min(global_step / 10000.0, 1.0)
      
      x, x_mask_output, mid_output, eeg_decoder_out, sid_logits = eeg_module(x, alpha)
      # For LaBraM: all tokens are valid (whole trimmed signal was patched uniformly),
      # so length = mid_output.size(2) for every item in the batch.
      # For CNN encoder: scale proportionally to the original EEG length.
      if eeg_decoder_out is None:  # LaBraM path
        mid_output_lengths = torch.full((mid_output.size(0),), mid_output.size(2),
                                        dtype=torch.long, device=mid_output.device)
      else:
        mid_output_lengths = (x_lengths.float() * mid_output.size(2) / x.size(2)).long()
      ...
      # Get phoneme predictions and speech decoder output
      y_hat, _, _, ids_slice, x_mask, z_mask, (z, z_p, m_p, logs_p, m_q, logs_q), phoneme_preds_speech = net_g(mid_output.detach(), mid_output_lengths, spec, spec_lengths, phoneme, alpha)

      loss_ctc_speech = torch.tensor(0.0).cuda(rank)
      if phoneme_preds_speech is not None:
        loss_ctc_speech = torch.nn.CTCLoss().cuda()(phoneme_preds_speech.transpose(0,1), phoneme[:,1:], phoneme_lengths-1, phoneme_lengths-1)

      y_mel = commons.slice_segments(spec_to_mel_torch(
          spec, 
          hps.data.filter_length, 
          hps.data.n_mel_channels, 
          hps.data.sampling_rate,
          hps.data.mel_fmin, 
          hps.data.mel_fmax), ids_slice, hps.train.segment_size // hps.data.hop_length)
    ...
    with autocast(enabled=False):
      ...
      with autocast(enabled=False):
        ...
        loss_eeg_module = eeg_loss(x, eeg_decoder_out, x_lengths) if eeg_decoder_out is not None else torch.tensor(0.0).cuda(rank)
        
        # Subject Adversarial Loss
        loss_sid = torch.tensor(0.0).cuda(rank)
        if sid is not None and sid_logits is not None:
          loss_sid = F.cross_entropy(sid_logits, sid)

        loss_eeg_phone = loss_eeg_module + hps.train.c_ctc * loss_ctc + (hps.train.c_sid if hasattr(hps.train, 'c_sid') else 1.0) * loss_sid

    # Combine generator and EEG/phone losses for a single backward pass
    total_loss = loss_gen_all + loss_eeg_phone + hps.train.c_ctc * loss_ctc_speech
    optim_g.zero_grad()
    optim_eeg.zero_grad()
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        print(f'\n=== Epoch {epoch} | Step {global_step} | Progress: {100. * batch_idx / len(train_loader):.1f}% ===')
        print(f'Discriminator Loss: {safe_item(loss_disc):.4f}')
        print(f'Generator Loss:     {safe_item(loss_gen):.4f}')
        print(f'Feature Match Loss: {safe_item(loss_fm):.4f}')
        print(f'Mel Loss:           {safe_item(loss_mel):.4f}')
        print(f'KL Loss:            {safe_item(loss_kl):.4f}')
        print(f'EEG Loss:           {safe_item(loss_eeg_module):.4f}')
        print(f'CTC Loss (EEG):     {safe_item(loss_ctc):.4f}')
        print(f'CTC Loss (Speech):  {safe_item(loss_ctc_speech):.4f}')
        print(f'Subject Discriminator Loss:       {safe_item(loss_sid):.4f}')
        print(f'Learning Rate:      {lr:.6f}')
        print(f'Grad Norm D:        {safe_item(grad_norm_d):.4f}')
        print(f'Grad Norm G:        {safe_item(grad_norm_g):.4f}')
        print('=' * 60)
        
        logger.info('Train Epoch: {} [{:.0f}%]'.format(
          epoch,
          100. * batch_idx / len(train_loader)))
        logger.info([safe_item(x) for x in losses] + [global_step, lr])
        
        scalar_dict = {"loss/g/total": loss_gen_all, "loss/d/total": loss_disc_all, "learning_rate": lr, "grad_norm_d": grad_norm_d, "grad_norm_g": grad_norm_g, "loss/eeg/total":loss_eeg_module ,"loss/ctc/total":loss_ctc, "loss/ctc_speech/total":loss_ctc_speech, "loss/sid/total": loss_sid
                       }
...
def evaluate(hps, net_infer, eval_loader, writer_eval, val_type):
    ...
    with torch.no_grad():
      ...
      x, x_mask_output, mid_output, eeg_decoder_out, sid_logits = eeg_module(x)
      # Same length fix as training: LaBraM outputs uniform-length sequences
      if eeg_decoder_out is None:  # LaBraM path
        mid_output_lengths = torch.full((mid_output.size(0),), mid_output.size(2),
                                        dtype=torch.long, device=mid_output.device)
      else:
