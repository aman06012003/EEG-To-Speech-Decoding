...
def run(rank, n_gpus, hps):
  ...
  if hps.train.pretrained_audio:
    ...
    for k, v in net_g.module.state_dict().items():
      # Skip enc_proj, subject_discriminator, speech_subject_discriminator, and speech_phoneme_predictor keys when loading pretrained weights
      if k.split('.')[0] in ['enc_proj', 'subject_discriminator', 'speech_subject_discriminator', 'speech_phoneme_predictor']:
        new_state_dict[k] = v
      else:
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, batch in enumerate(train_loader):
    ...
    with autocast(enabled=False):
      ...
      # Get subject logits from speech module
      y_hat, _, _, ids_slice, x_mask, z_mask, (z, z_p, m_p, logs_p, m_q, logs_q), sid_logits_speech, speech_phoneme_preds = net_g(mid_output.detach(), mid_output_lengths, spec, spec_lengths, alpha)
    ...
    with autocast(enabled=False):
      ...
      with autocast(enabled=False):
        ...
        loss_subj = torch.nn.CrossEntropyLoss()(sid_logits, sid) if sid is not None else torch.tensor(0.0).cuda()
        # Subject Adversarial Loss (Speech)
        loss_subj_speech = torch.nn.CrossEntropyLoss()(sid_logits_speech, sid) if sid is not None else torch.tensor(0.0).cuda()
        c_subj = hps.train.c_subj if hasattr(hps.train, 'c_subj') else 1.0
        
        # Speech phoneme predictor loss (CTC)
        speech_phoneme_preds = speech_phoneme_preds  # from net_g forward
        loss_ctc_speech = torch.nn.CTCLoss().cuda()(speech_phoneme_preds.transpose(0,1), phoneme[:,1:], phoneme_lengths-1, phoneme_lengths-1)

        # Add to total loss
        loss_eeg_phone = loss_eeg_module + hps.train.c_ctc * loss_ctc + c_subj * loss_subj + c_subj * loss_subj_speech + hps.train.c_ctc * loss_ctc_speech

    # Combine generator and EEG/phone losses for a single backward pass
    total_loss = loss_gen_all + loss_eeg_phone
    optim_g.zero_grad()
    optim_eeg.zero_grad()
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        # Subject adversarial loss and entropy
        if sid is not None and sid_logits is not None:
          loss_subj_disc = loss_subj
          sid_probs = torch.nn.functional.softmax(sid_logits, dim=-1)
          loss_subj_entropy = (-(sid_probs * torch.log(sid_probs + 1e-8)).sum(dim=-1).mean())
        else:
          loss_subj_disc = torch.tensor(0.0)
          loss_subj_entropy = torch.tensor(0.0)
        ...
        print(f'CTC Loss:           {safe_item(loss_ctc):.4f}')
        print(f'Subject Disc Loss (EEG):  {safe_item(loss_subj_disc):.4f}')
        print(f'Subject Disc Loss (Speech):  {safe_item(loss_subj_speech):.4f}')
        print(f'Subject Entropy:    {safe_item(loss_subj_entropy):.4f}')  
